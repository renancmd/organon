...
// Assumes you have updateTask and deleteTask in your service!
...
export default function EditTodoModal({ isOpen, onClose, taskId }: EditTodoModalProps) {
    ...
    // Fetch areas AND the specific task data when the modal opens
    useEffect(() => {
        ...
        const loadData = async () => {
            setIsLoading(true);
            try {
                // Fetch both areas and tasks concurrently
                ...
                // Find the specific task we are editing
                ...
                if (taskToEdit) {
                    ...
                    if (matchedArea) {
                        ...
                        setSelectedArea((fetchedAreas as Area[])[0].id); // Fallback
                    }

                    // Map subtasks to local state format
    ...
    const handleSaveChanges = async () => {
        ...
        const formattedSubtasks = subtasks.map((st, index) => ({
            id: st.id.includes("sub-") ? st.id : `sub-${Date.now()}${index}`, // Keep existing IDs if they have them
            name: st.title,
            completed: false // You might want to preserve actual completion status if you track it
        }));
