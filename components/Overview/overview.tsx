...
export default function Overview() {
	...
	// NOTE: Removed the outer <div className="mx-auto..."> wrapper here
	return (
		<div className="flex flex-col h-full rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-colors duration-300 dark:border-gray-800 dark:bg-[#181818]">
			...
			<div className="mb-6 flex flex-row gap-3 overflow-x-auto pb-2 scrollbar-hide">
				<TabButton label="Today" isActive={activeTab === "today"} onClick={() => setActiveTab("today")} />
				<TabButton label="Tomorrow" isActive={activeTab === "tomorrow"} onClick={() => setActiveTab("tomorrow")} />
				<TabButton label="Next 7 days" isActive={activeTab === "next7"} onClick={() => setActiveTab("next7")} />
			</div>

			<div className="flex-1 pt-6 border-t border-gray-100 dark:border-gray-800/60">
			...
			<EditTodoModal isOpen={!!editingTaskId} taskId={editingTaskId} onClose={() => { setEditingTaskId(null); fetchDashboardData(true); }} />
			<EditEventModal isOpen={!!editingEventId} eventId={editingEventId} onClose={() => setEditingEventId(null)} onSuccess={() => { setEditingEventId(null); fetchDashboardData(true); }} />
		</div>
	);
}
