    ...
    return (
        <aside className="fixed inset-y-0 left-0 z-40 hidden w-20 flex-col border-r bg-white dark:bg-gray-950 dark:border-gray-800 md:flex">
            <nav className="flex flex-col items-center gap-4 px-2 sm:py-5">
                ...
                <a href="/habitos" className="flex h-9 w-9 items-center justify-center rounded-lg text-gray-500 transition-colors hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50"><Repeat className="h-5 w-5" /></a>
                {/* O link do Diário (Linha do Tempo) agora está ativo */}
                <a href="/daily-journal" className="flex h-9 w-9 items-center justify-center rounded-lg bg-gray-100 text-gray-900 transition-colors hover:text-gray-900 dark:bg-gray-800 dark:text-gray-50 dark:hover:text-gray-50"><GanttChart className="h-5 w-5" /></a>
            </nav>
...
export default function DailyJournalLayout({ children }: { children: React.ReactNode }) {
  return (
